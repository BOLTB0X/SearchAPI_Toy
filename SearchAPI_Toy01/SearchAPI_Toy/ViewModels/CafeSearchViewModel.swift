...
// MARK: - CafeSearchViewModel
class CafeSearchViewModel: ObservableObject {
    @Published var searchCafes: [CafeDocument] = [] // 검색된 문서를 담아둘 배열
    ...
    // MARK: - fetchCafeSearchData
    func fetchCafeSearchData(query: String) {
        guard !endPage else {
            print("마지막 페이지")
            return
        }
        ...
        guard let request = NetworkManager.RequestURL(Url: APIEndpoint.cafe.path, searchParam: searchParam) else {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) { [weak self] in
            ...
            CafeSearchManager.shared
                .CafeSearchPublisher(dataPublisher: dataPublisher)
                .sink(receiveCompletion: { [weak self] completion in
                    self?.cafeOnReeive(completion)
                }, receiveValue: { [weak self] response in
                    self?.cafeOnReeive(response)
                })
    ...
    // MARK: - checkFetchMore
    func checkFetchMore(document: CafeDocument) {
        // 현재 document가 마지막이면
        if !searchCafes.isEmpty &&
            document == searchCafes.last {
            fetchCafeSearchData(query: inputText)
            return
        }
        return
    }
    ...
    // MARK: - cafeOnReeive
    private func cafeOnReeive(_ completion: Subscribers.Completion<Error>) {
        switch completion {
    ...
    private func cafeOnReeive(_ response: CafeResponse) {
        // 이제 내가 필요한 것들을 작업
        self.currentPage += 1
        ...
        self.searchCafes.append(contentsOf: response.documents) // 검색결과 배열에 넣어줌
    }
}
