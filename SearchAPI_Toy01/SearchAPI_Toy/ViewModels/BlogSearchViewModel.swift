//
//  BlogSearchViewModel.swift
//  SearchAPI_Toy
//
//  Created by KyungHeon Lee on 2023/06/22.
//

import Foundation
import Combine

// MARK: - BlogSearchViewModel
class BlogSearchViewModel: ObservableObject {
    @Published var searchBlogs: [BlogDocument] = [] // 검색된 문서를 담아둘 배열
    ...
    // MARK: - fetchBlogSearchData
    func fetchBlogSearchData(query: String) {
        guard !endPage else {
            print("마지막 페이지")
            return
        }
        ...
        guard let request = NetworkManager.RequestURL(Url: APIEndpoint.blog.path, searchParam: searchParam) else {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2) { [weak self] in
            ...
            BlogSearchManager.shared
                .BlogSearchPublisher(dataPublisher: dataPublisher)
                .sink(receiveCompletion: { [weak self] completion in
                    self?.blogOnReeive(completion)
                }, receiveValue: { [weak self] response in
                    self?.blogOnReeive(response)
                })
    ...
    // MARK: - checkFetchMore
    func checkFetchMore(document: BlogDocument) {
        // 현재 document가 마지막이면
        if !searchBlogs.isEmpty &&
            document == searchBlogs.last {
            fetchBlogSearchData(query: inputText)
            return
        }
        return
    }
    ...
    // MARK: - cafeOnReeive
    private func blogOnReeive(_ completion: Subscribers.Completion<Error>) {
        switch completion {
    ...
    private func blogOnReeive(_ response: BlogResponse) {
        // 이제 내가 필요한 것들을 작업
        self.currentPage += 1
        ...
        self.searchBlogs.append(contentsOf: response.documents) // 검색결과 배열에 넣어줌
    }
}
